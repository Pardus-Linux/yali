...
allParameters = ["initrd","init","xorg","yali","BOOT_IMAGE","lang","mudur"]
...
def get_commands(storage):
    ...
    if storage.swapDevices:
        _commands.append("resume=%s" % storage.swapDevices[0].path)

    for command in [cmd for cmd in open("/proc/cmdline", "r").read().split()]:
        for parameter in allParameters:
            if command.startswith("%s=" % parameter):
...
class BootLoader(object):
    _grubConf = "grub/grub.conf"
    _deviceMap = "grub/device.map"
    def __init__(self, storage=None):
        self.storage = storage
        self._path = None
        self._device = None
        self._type = BOOT_TYPE_NONE
        self.otherEnabled = False
    ...
    def availablesDevices(self, root=True):
        ...
        for partition in [p for p in self.storage.partitions if p.exists]:
            ...
            if partition.format.type in dos_filesystems and check_dual_boot() \
                    and not partition.getFlag(parted.PARTITION_DIAG):
                try:
    ...
    def setup(self):
        grubtarget = self.device
        bootDevice = self.storage.storageset.bootDevice
        if bootDevice.mountpoint == "/boot":
            self.path = "/"
        else:
            self.path = "/boot/"

        self.write(grubtarget, bootDevice)

    def write(self, grubtarget, bootDevice):

        self.writeGrubConf(bootDevice, others=True)

        usedDevices = set()
        usedDevices.update(get_physical_devices(self.storage, self.storage.devicetree.getDeviceByName(grubtarget)))
        usedDevices.update(get_physical_devices(self.storage, bootDevice))
        usedDevices.update([device for device, (label, filesystem) in self.others.items()])

        self.writeDeviceMap(usedDevices)
        self.install(bootDevice, grubtarget)

    def writeGrubConf(self, bootDevice, others=False):
        bootDevices = get_physical_devices(self.storage, bootDevice)
        ...
        open(self._grubConf, "w").write(s)

    def writeDeviceMap(self, usedDevices):
        f = open(self._deviceMap, "w+")
        f.write("# this device map was generated by YALI\n")
        usedDisks = set()
    ...
    def install(self, bootDevice, stage1target):
        grubpath = "%sgrub" % self.path
        stage1Devices = get_physical_devices(self.storage, self.storage.devicetree.getDeviceByName(stage1target))
        bootDevices = get_physical_devices(self.storage, bootDevice)
...
""" % (bootPartitionPath, grubpath, stage1Path, grubpath, bootPartitionPath, grubpath)
        ...
        rc = yali.util.run_batch("grub", ["--no-floppy"], ["--batch <"], ["/tmp/batch"])

        if not rc:
            yali.util.run_batch("grub", ["--no-floppy"], ["--batch <"], ["/tmp/batch"])
