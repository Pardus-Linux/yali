...
class BootLoader:
    ...
    def write_grub_conf(self, install_root_path,install_dev):
        ...
        def boot_parameters(root):
            ...
            for i in [x for x in open("/proc/cmdline", "r").read().split() if not x.startswith("init=") and not x.startswith("xorg=") and not x.startswith("yali=") and not x.startswith(ctx.kahyaParam)]:
